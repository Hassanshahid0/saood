...
FROM tbl_food_menus fm  INNER JOIN (select * from tbl_food_menu_categories where del_status='Live') fmc ON fmc.id = fm.category_id LEFT JOIN (select * from tbl_sales_details where del_status='Live') sd ON sd.food_menu_id = fm.id WHERE FIND_IN_SET(fm.id, '$getFM') AND fm.del_status='Live' GROUP BY fm.id order BY name ASC")->result();
    ...
    public function top_ten_food_menu($start_date, $end_date,$outlet_id='') {
        $this->db->select('sum(qty) as totalQty, sum(menu_price_with_discount * qty) as totalRevenue, food_menu_id, menu_name, sale_date');
     ...
     /**
     * get top 5 categories by quantity sold
     * @access public
     * @return array
     * @param string
     * @param string
     * @param int
     */
    public function top_five_categories($start_date, $end_date, $outlet_id='') {
        $company_id = $this->session->userdata('company_id');
        $query = "SELECT 
                    fmc.category_name,
                  ...
                  LIMIT 5";
        $result = $this->db->query($query, array($company_id, $outlet_id, $start_date, $end_date));
    ...
    public function top_ten_customer($start_date, $end_date,$outlet_id='') {
        $company_id = $this->session->userdata('company_id');
        $this->db->select('sum(tbl_sales.total_payable) as total_payable, count(tbl_sales.id) as total_visits, MAX(tbl_sales.sale_date) as last_visit_date, tbl_sales.customer_id, tbl_customers.name, tbl_sales.sale_date,tbl_customers.phone');
    ...
    public function customer_receivable($outlet_id) {
        $this->db->select('sum(due_amount) as due_amount, customer_id, name, tbl_customers.phone, MIN(sale_date) as sale_date');
        $this->db->from('tbl_sales');
    ...
    public function supplier_payable($outlet_id) {
        $this->db->select('sum(due) as due, supplier_id, name, phone, MIN(date) as purchase_date');
     ...
     * @param int
     */
    public function get_order_type_data($start_date, $end_date, $outlet_id='') {
        $company_id = $this->session->userdata('company_id');
        $query = "SELECT 
                    CASE s.order_type
                        WHEN 1 THEN 'Dine In'
        ...
        $result = $this->db->query($query, array($start_date, $end_date, $company_id, $outlet_id));
