...
function escape_output($string){
    ...
    function getExistFoodMenuKitchen($sale_id,$food_menu_id) {
        $CI = & get_instance();
        $CI->db->select('*');
    ...
    function isServiceAccess($user_id='',$company_id='',$service_type='') {
        ...
        if($service_type && $service_type =="fTzfWnSWR" && str_rot13($company->languge_manifesto?$company->languge_manifesto:'xx') =="fTzfWnSWIR" &&  file_exists(APPPATH.'controllers/Service.php')){
    ...
    function isServiceAccessOnly($service_type='') {
        ...
        $status = false;
        $company_id = $CI->session->userdata("company_id");
        if($company_id==1 && $service_type && $service_type =="fTzfWnSWR" && str_rot13($company->languge_manifesto?$company->languge_manifesto:'xx') =="fTzfWnSWIR" &&  file_exists(APPPATH.'controllers/Service.php')){
            return true;
        }else{
            $service_type = str_rot13($service_type);
            if($service_type && $service_type =="fTzfWnSWR" && str_rot13($company->languge_manifesto?$company->languge_manifesto:'xx') =="fTzfWnSWIR" &&  file_exists(APPPATH.'controllers/Service.php')){
    ...
    function generateSaleItemsArray($items,$partner_id)
{
    $final = [];
    ...
    foreach ($items as $row) {
        ...
        if (isset($row->modifiers) && is_array($row->modifiers)) {
            foreach ($row->modifiers as $mod) {

                // Convert modifier object safely
                $modifiers[] = [
                    "modifier_id"   => isset($mod->modifier_id) ? $mod->modifier_id : "",
                    "modifier_name" => isset($mod->name) ? $mod->name : "",
                    "modifier_price"=> isset($mod->modifier_price) ? $mod->modifier_price : ""
                ];
            }
        }
        ...
        // --- Build Each Item ---
        $final[] = [
            "food_menu_id"  => $row->food_menu_id ?? "",
            "unique_id"     => uniqid(),
            "order_type"    => "dine_in",
            "code"          => $row->code ?? $row->food_menu_id ?? "",
            "product_name"  => $row->menu_name ?? "",
            "quantity"      => $row->qty ?? 1,
            "discount_type" => $row->discount_type == "fixed" ? 1: 2,
            "discount"      => $row->menu_discount_value ?? "",
            "product_image" => "",
            "pratner_id" => $partner_id,
            "sale_price"    => $row->menu_price_with_discount ?? 0,
            "modifiers"     => $modifiers,
            "note"          => $row->menu_note ?? ""
        ];
    }
    ...
    function generateCartItemsFromDB($items)
{
    $final = [];
    ...
    foreach ($items as $row) {
        ...
        $modifiers = [];
        if (!empty($row->modifiers) && is_array($row->modifiers)) {
            foreach ($row->modifiers as $mod) {
                $modifiers[] = [
        ...
        // Build the required item array
        $final[] = [
            "food_menu_id" => $row->food_menu_id,
            "unique_id" => uniqid(),   // no unique_id available → generate
            "order_type" => "dine_in",
            "code" => $row->food_menu_id,
            "product_name" => $row->menu_name,
            "quantity" => $row->qty,
            "discount_type" => $row->discount_type == "fixed" ? 1: 2,
            "discount" => $row->menu_discount_value ?? "",
            "product_image" => "",     // not available in DB
            "sale_price" => $row->menu_price_with_discount,
            "modifiers" => $modifiers,
            "note" => $row->menu_note
        ];
    }
    ...
    function getOrderDetailsForEdit($row_id) {
        $CI = & get_instance();
        $data = $CI->db->query("SELECT * FROM tbl_kitchen_sales WHERE id='$row_id' AND del_status='Live'")->row();
        if($data){
            return $data;
        }else{
            return false;
    ...
    function getOrderDetailsForEditSale($row_id) {
        $CI = & get_instance();
        $data = $CI->db->query("SELECT * FROM tbl_sales WHERE id='$row_id' AND del_status='Live'")->row();
        if($data){
            return $data;
        }else{
            return false;
    ...
    function buildOrderItemsArray($jsonString) {
        // Decode the main sale JSON (string from DB)
        ...
        foreach ($data['items'] as $item) {
            ...
            // Prepare modifiers array
            $modifiers = [];
    
            if (!empty($item['modifiers_id']) && !empty($item['modifiers_name']) && !empty($item['modifiers_price'])) {
    
                $ids = explode(",", $item['modifiers_id']);
                $names = explode(",", $item['modifiers_name']);
                $prices = explode(",", $item['modifiers_price']);
    
                // Build modifier objects
                for ($i = 0; $i < count($ids); $i++) {
                    $modifiers[] = [
                        "modifier_id" => trim($ids[$i]),
                        "modifier_name" => trim($names[$i] ?? ""),
                        "modifier_price" => trim($prices[$i] ?? "")
                    ];
                }
            }
    
            // Build each item in required structure
            $final[] = [
                "food_menu_id" => $item["food_menu_id"],
                "unique_id" => isset($item["unique_id"]) ? $item["unique_id"] : uniqid(),
                "order_type" => "dine_in",
                "code" => $item["food_menu_id"],  
                "product_name" => $item["menu_name"],
                "quantity" => $item["qty"],
                "discount_type" => $item["discount_type"],
                "discount" => $item["menu_discount_value"] ?? "",
                "product_image" => "", // not available in your JSON
                "sale_price" => $item["menu_price_with_discount"],
                "modifiers" => $modifiers,
                "note" => $item["item_note"] ?? ""
            ];
        }
    ...
    */
    ...
    function getFoodMenuForMenuPage() {
        $CI = & get_instance();
        $online_selected_outlet_updated = $CI->session->userdata('online_selected_outlet_updated');
        
        
        $outlet_details = getOutletById($online_selected_outlet_updated);
        if($outlet_details){
    ...
    function getTotalLoyaltyPoint($id,$outlet_id) {
        ...
        return [(int)$total_usage, (int)$total_point];
    ...
    /**
 * Merge multiple POS order JSON arrays into one
 * First JSON is treated as master
 */
// function merge_multiple_pos_orders(array $orders): array
// {
...
//     // take first order as base
//     $merged = array_shift($orders);
//     $merged['is_merge'] = '1';
    
//     foreach ($orders as $order) {

//         /* ----------------------------
//            SALE NO (comma separated)
//         ----------------------------- */
//         $merged['sale_no'] .= ',' . $order['sale_no'];
...
//             $key = (string)$item['food_menu_id'];

//             $vatMap = [];
//             foreach ($item['item_vat'] ?? [] as $vat) {
//                 $vatKey = $vat['tax_field_name'] ?? $vat['tax_field_type'];
//                 $vatMap[$vatKey] = $vat;
//             }
//             $item['item_vat'] = $vatMap;
//             $itemsMap[$key] = $item;
//         }
...
//             $itemsMap[$key]['qty'] += (int)$item['qty'];

//             $itemsMap[$key]['menu_price_with_discount'] =
//                 $add($itemsMap[$key]['menu_price_with_discount'], $item['menu_price_with_discount']);

//             $itemsMap[$key]['menu_price_without_discount'] =
//                 $add($itemsMap[$key]['menu_price_without_discount'], $item['menu_price_without_discount']);

//             $itemsMap[$key]['item_discount_amount'] =
//                 $add($itemsMap[$key]['item_discount_amount'], $item['item_discount_amount']);
...
//                     $vatAmount =
//                         $vat['item_vat_amount_for_all_quantity']
//                         ?? $vat['tax_field_amount']
//                         ?? 0;
                
//                     if (isset($itemsMap[$key]['item_vat'][$vatKey])) {
                
//                         $existingAmount =
//                             $itemsMap[$key]['item_vat'][$vatKey]['item_vat_amount_for_all_quantity']
//                             ?? $itemsMap[$key]['item_vat'][$vatKey]['tax_field_amount']
//                             ?? 0;
                
//                         // update BOTH fields safely
//                         $itemsMap[$key]['item_vat'][$vatKey]['item_vat_amount_for_all_quantity'] =
//                             $add($existingAmount, $vatAmount);
                
//                     } else {
//                         // normalize structure for consistency
//                         $vat['item_vat_amount_for_all_quantity'] = $vatAmount;
//                         $itemsMap[$key]['item_vat'][$vatKey] = $vat;
//                     }
//                 }
...
//             $item['item_vat'] = array_values($item['item_vat']);
//         }

//         $merged['items'] = array_values($itemsMap);
...
//         foreach ($merged['sale_vat_objects'] ?? [] as $vat) {
//             $saleVatMap[$vat['tax_field_type']] = $vat;
//         }

//         foreach ($order['sale_vat_objects'] ?? [] as $vat) {
//             $key = $vat['tax_field_type'];

//             if (isset($saleVatMap[$key])) {
//                 $saleVatMap[$key]['tax_field_amount'] =
//                     $add($saleVatMap[$key]['tax_field_amount'], $vat['tax_field_amount']);
//             } else {
//                 $saleVatMap[$key] = $vat;
//             }
//         }

//         $merged['sale_vat_objects'] = array_values($saleVatMap);
...
//             if (isset($merged[$field], $order[$field])) {
//                 $merged[$field] = $add($merged[$field], $order[$field]);
//             }
//         }

//         $merged['total_items_in_cart'] =
//             (int)$merged['total_items_in_cart'] + (int)$order['total_items_in_cart'];

//         $merged['total_items_in_cart_qty'] =
//             (int)$merged['total_items_in_cart_qty'] + (int)$order['total_items_in_cart_qty'];
//     }

//     // force common data from first order
//     foreach (['customer_id', 'customer_name', 'order_type', 'order_status'] as $field) {
...
function merge_multiple_pos_orders(array $orders): array
{
    ...
    foreach ($orders as $order) {
        ...
        foreach ($order['items'] as $item) {
            ...
            /* -------- MODIFIERS (STRING BASED) -------- */
            $existingModifierIds   = array_filter(explode(',', $itemsMap[$key]['modifiers_id'] ?? ''));
            $existingModifierNames = array_filter(explode(',', $itemsMap[$key]['modifiers_name'] ?? ''));
            $existingModifierPrice = array_filter(explode(',', $itemsMap[$key]['modifiers_price'] ?? ''));

            $existingModifiers = [];

            foreach ($existingModifierIds as $idx => $modId) {
                $existingModifiers[$modId] = [
                    'name'  => $existingModifierNames[$idx] ?? '',
                    'price' => $existingModifierPrice[$idx] ?? ''
                ];
            }

            $newModifierIds   = array_filter(explode(',', $item['modifiers_id'] ?? ''));
            $newModifierNames = array_filter(explode(',', $item['modifiers_name'] ?? ''));
            $newModifierPrice = array_filter(explode(',', $item['modifiers_price'] ?? ''));

            foreach ($newModifierIds as $idx => $modId) {
                if (!isset($existingModifiers[$modId])) {
                    $existingModifiers[$modId] = [
                        'name'  => $newModifierNames[$idx] ?? '',
                        'price' => $newModifierPrice[$idx] ?? ''
                    ];
                }
            }

            $itemsMap[$key]['modifiers_id']    = implode(',', array_keys($existingModifiers));
            $itemsMap[$key]['modifiers_name']  = implode(',', array_column($existingModifiers, 'name'));
            $itemsMap[$key]['modifiers_price'] = implode(',', array_column($existingModifiers, 'price'));
        }

        // normalize item_vat back to array
        foreach ($itemsMap as &$item) {
            $item['item_vat'] = array_values($item['item_vat']);
        }
    ...
    function getSelectedModifiersByModiersIds($modifier_ids){
        if($modifier_ids){  
            ...
            /* Normalize modifier_ids */
            if (empty($modifier_ids)) {
                return [];
            }

            if (is_string($modifier_ids)) {

                // JSON string like "[5,6]"
                $decoded = json_decode($modifier_ids, true);

                if (json_last_error() === JSON_ERROR_NONE) {
                    $modifier_ids = $decoded;
                } else {
                    // fallback: "5,6"
                    $modifier_ids = explode(',', $modifier_ids);
                }
            }

            $modifier_ids = array_map('intval', (array) $modifier_ids);
    ...
    foreach ($database_response as $item) {
        ...
        if (!empty($modifiers) && (is_array($modifiers) || is_object($modifiers))) {
