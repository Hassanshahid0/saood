...
class Authentication extends Cl_Controller {
  ...
  public function frontend() {
        ...
        if($is_valid){
            ...
            if(isServiceAccessOnly('sGmsJaFJE')){
                ...
                if(str_rot13($company->language_manifesto)=="eriutoeri"){
                    ...
                    $this->session->set_userdata('online_selected_outlet_updated', 1);
   ...
   public function loginCheck() {
    ...
    // Only allow POST submit
    if ($this->input->post('submit') != 'submit') {
        redirect("Authentication/index");
    }

    // Load required helpers/models (prevent blank page)
    $this->load->helper(['form', 'url', 'security']);
    $this->load->library('form_validation');
    $this->load->model(['Authentication_model', 'Common_model']);

    // Get active login mode (1 = Username/Password, 2 = PIN)
    $active_login_button = $this->input->post('active_login_button_hidden') ?? 1;
    ...
    if ($this->form_validation->run() == TRUE) {
        ...
        $login_pin = $this->input->post($this->security->xss_clean('login_pin'));

        // Fetch user info (pass PIN and mode)
        $user_information = $this->Authentication_model->getUserInformation(
            $email_address, 
            $password, 
            $login_pin, 
            $active_login_button
        ...
        // User exists?
        if ($user_information) {
            insertSosUser(); // SOS insert if needed
                ...
                if ($company_info && $company_info->is_active == 1) {
                    ...
                    if (!isFoodCourt()) {
                        if (!isServiceAccess($user_information->id, $user_information->company_id) && $user_information->company_id != 1) {
                            ...
                            if ($due_payment && $due_payment->payment_date) {
                                $access_day = $company_info->access_day ?: 0;
                                $today = date("Y-m-d");
                    ...
                    if ($is_payment_clear == "Yes" && $is_block == "No") {
                        ...
                        if ($user_information->role == "Admin") {
                            ...
                            if (!empty($getAccess)) {
                                foreach ($getAccess as $value) {
                                    ...
                                    if ($getAccesRow) {
                                        $menu_access_container[] = $getAccesRow->function_name . "-" . $getAccesRow->parent_id;
                                    }
                                }
                            }
                        }
                        ...
                        $login_session = [
                            ...
                            'designation' => $user_information->designation,
                            'full_name' => $user_information->full_name,
                            'short_name' => strtolower(substr($user_information->full_name, 0, 1)),
                            ...
                            'session_outlets' => $user_information->outlets,
                            'active_menu_tmp' => 4
                        ];

                        // Company session data (unchanged)
                        $company_info_session = [
                            'currency' => $company_info->currency,
                            'zone_name' => $company_info->zone_name,
                            'date_format' => $company_info->date_format,
                            'business_name' => $company_info->business_name,
                            'website' => $company_info->website,
                            'currency_position' => $company_info->currency_position,
                            'precision' => $company_info->precision,
                            'default_customer' => $company_info->default_customer,
                            'export_daily_sale' => $company_info->export_daily_sale,
                            'service_amount' => $company_info->service_amount,
                            'take_away_service_charge' => $company_info->take_away_service_charge,
                            'delivery_amount' => $company_info->delivery_amount,
                            'tax_type' => $company_info->tax_type,
                            'decimals_separator' => $company_info->decimals_separator,
                            'thousands_separator' => $company_info->thousands_separator,
                            'open_cash_drawer_when_printing_invoice' => $company_info->open_cash_drawer_when_printing_invoice,
                            'when_clicking_on_item_in_pos' => $company_info->when_clicking_on_item_in_pos,
                            'is_rounding_enable' => $company_info->is_rounding_enable,
                            'attendance_type' => $company_info->attendance_type,
                            'default_order_type' => $company_info->default_order_type,
                            'is_loyalty_enable' => $company_info->is_loyalty_enable,
                            'pre_or_post_payment' => $company_info->pre_or_post_payment,
                            'minimum_point_to_redeem' => $company_info->minimum_point_to_redeem,
                            'loyalty_rate' => $company_info->loyalty_rate,
                            'split_bill' => $company_info->split_bill,
                            'place_order_tooltip' => $company_info->place_order_tooltip,
                            'food_menu_tooltip' => $company_info->food_menu_tooltip,
                            'table_bg_color' => $company_info->table_bg_color,
                            'kot_print_online_self_order' => $company_info->kot_print_online_self_order,
                            'default_waiter' => (str_rot13($company_info->language_manifesto) != "eriutoeri") ? $company_info->default_waiter : null,
                            'default_payment' => $company_info->default_payment,
                            'invoice_footer' => $company_info->invoice_footer,
                            'invoice_logo' => $company_info->invoice_logo,
                            'language_manifesto' => $company_info->language_manifesto,
                            'collect_tax' => $company_info->collect_tax,
                            'tax_title' => $company_info->tax_title,
                            'tax_registration_no' => $company_info->tax_registration_no,
                            'tax_is_gst' => $company_info->tax_is_gst,
                            'apply_on_delivery_charge' => $company_info->apply_on_delivery_charge,
                            'state_code' => $company_info->state_code,
                            'active_login_button' => $company_info->active_login_button,
                            'login_type' => $company_info->login_type
                        ];

                        $this->session->set_userdata($login_session);
                        $this->session->set_userdata($company_info_session);

                        // Outlet handling (unchanged)
                        if (isFoodCourt() && $user_information->id != 1) {
                            $outlet_query = $user_information->outlet_id 
                                ? "id = {$user_information->outlet_id}" 
                                : "del_status='Live' AND active_status='active'";
                            $outlet_info = $this->db->query("SELECT * FROM tbl_outlets WHERE del_status='Live' AND active_status='active' AND $outlet_query")->row();
                        } else {
                        ...
                        if (!empty($outlet_info)) {
                            $outlet_session = [
                                ...
                                'has_kitchen' => $outlet_info->has_kitchen,
                                'online_order_module' => $outlet_info->online_order_module,
                                'default_waiter' => (str_rot13($company_info->language_manifesto) == "eriutoeri") ? $outlet_info->default_waiter : null
                            ];
                        ...
                        $error_msg = ($is_block == "Yes") ? lang('block_tmp_err') : lang('payment_not_clear_err');
                        $this->session->set_flashdata('exception_1', $error_msg);
            ...
            $this->session->set_flashdata('exception_1', ($active_login_button == 1) ? lang('incorrect_email_password') : lang('incorrect_pin'));
            $this->session->set_flashdata('active_login_button', $active_login_button);
            redirect('Authentication/index');
        }
    } else {
    ...
    public function logout_online_order() {
        ...
        $this->session->unset_userdata('mobile_responsive_checker_self_order');
        $this->session->unset_userdata('short_name');
        $this->session->unset_userdata('online_selected_outlet_updated');
  ...
  public function setOutlet($outlet_id,$company_id){
    ...
    public function setOutletForOnline()
    {
        ...
        $this->session->set_userdata('online_selected_outlet_updated', $outlet_id); 
