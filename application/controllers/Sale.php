                ...
                foreach($order_details->items as $key_counter=>$item){
                    ...
                    if($sale_id){
                        ...
                        if(isset($check_exist_item) && $check_exist_item){
                            $holds_details_id = $check_exist_item->id;
                            ...
                            $this->Common_model->updateInformation($item_data, $holds_details_id, "tbl_kitchen_sales_details");
                        }else{
                    ...
                    $previous_food_id = $holds_details_id;
                    $update_previous_id = array();
                    $update_previous_id['previous_id'] = $previous_food_id;
                    $this->Common_model->updateInformation($update_previous_id, $holds_details_id, "tbl_kitchen_sales_details");
                    ...
                    if(!empty($modifier_id_array)>0){
                        ...
                        foreach($modifier_id_array as $key1=>$single_modifier_id){
                            ...
                            $modifier_data['sales_details_id'] = $holds_details_id;
                            ...
                            if($sale_id){
                                $check_exist_modifer = checkExistItemModifer($sale_id,$item->food_menu_id,$holds_details_id,$single_modifier_id);
                ...
                $exist_food_menu = getExistFoodMenu($sale_id_old_sale_id,$item->item_id);
                 if(isset($exist_food_menu) && $exist_food_menu->qty){
                     ...
                     if($item->item_previous_id==""){
                         $previous_id_update_array = array('previous_id' => $holds_details_id);
                         $this->db->where('id', $holds_details_id);
...
We hope to see you again!";
            ...
            if($select_kitchen_row){
                ...
                if($pre_or_post_payment==1){
                    ...
                    $this->db->where('id', $select_kitchen_row->id);
    ...
    public function add_hold_by_ajax()
    {
        ...
        $data['order_time'] = date('Y-m-d h:i A');
        $data['order_status'] = trim_checker($order_details->order_status);
    ...
    public function get_single_hold_info_by_ajax()
    {
        ...
        foreach($items_by_holds_id as $single_item_by_hold_id){
            $modifier_information = $this->Sale_model->getModifiersByHoldAndHoldsDetailsId($hold_id,$single_item_by_hold_id->sales_details_id);
    ...
    public function getAjaxData()
            {
                ...
                $i = isset($_POST['start']) ? (int)$_POST['start'] + count($sales) : count($sales);
                ...
                foreach ($sales as $value) {
                    ...
                    if (!empty($salePaymentDetails)) {
                        foreach ($salePaymentDetails as $k => $payment) {
                            $txt_point = ($payment->id == 5) ? " (Usage Point: {$payment->usage_point})" : '';
                            $payment_details .= escape_output($payment->payment_name.$txt_point).':'.escape_output(getAmtPCustom($payment->amount));
                            if ($k < count($salePaymentDetails) - 1) {
                                $payment_details .= ' - ';
                            }
                        }
                    }
