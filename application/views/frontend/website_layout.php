...
$online_selected_outlet_updated = $this->session->userdata('online_selected_outlet_updated');
$total = (is_array($get_total_outlet)) ? sizeof($get_total_outlet) : 0;
if($total==1){
  $online_selected_outlet_updated = $get_total_outlet[0]->id; 
  $this->session->set_userdata('online_selected_outlet_updated', $online_selected_outlet_updated);
}
...
<body class="<?php echo isset($language) && $language == "arabic" ? 'arabic-lang"' : '' ?>">
  ...
  <input type="hidden" id="online_selected_outlet_updated" value="<?php echo $online_selected_outlet_updated ?>">
  <!-- Preloader Start -->
...
?>
  ...
  <script src="<?php echo base_url() ?>assets/website/js/order.js?v=1.1"></script>
